...
private:
    ...
    // Колбэки для окон
    std::function<void(const std::string&)> toggle_window_callback_;
...
public:
    ...
    // Установка колбэков для окон
    void setToggleWindowCallback(std::function<void(const std::string&)> callback) {
        toggle_window_callback_ = std::move(callback);
    }
