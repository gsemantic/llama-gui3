...
public:
    ...
    // Отправка сообщения программно (как если бы ввёл пользователь).
    // Используется системой плагинов (host API chat_send_message).
    bool submit_message(const std::string& content);
