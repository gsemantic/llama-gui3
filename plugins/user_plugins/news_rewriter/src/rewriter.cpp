...
std::string build_role_from_template(const std::string& tpl,
    ...
    // SEO: инструкция органично вплетать ключевую фразу темы (focus_keyword)
    // в тело — иначе анализатор засчитывает плотность 0%, отсутствие фразы в
    // первом абзаце/подзаголовке и штрафует скоркард. На этапе рерайта сама
    // фраза ещё не известна (её выдаёт отдельный SEO-шаг), поэтому просим
    // модель использовать ключевую мысль из заголовка.
    role +=
        "\n\nДля SEO-оптимизации органично используй ключевую фразу по теме статьи "
        "(например, главную мысль из заголовка) как минимум в первом абзаце и в "
