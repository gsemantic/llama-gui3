...
`LlmFn` — тонкая обёртка над `g_api->llm_complete` / `llm_complete_ex` (живёт
только в worker). Промпт разделён на **роль** (системный промпт: инструкции,
персона, язык/тон/объём — статичен) и **контент статьи** (user-сообщение:
`Заголовок:`/`Текст:`). Роль собирается РОВНО ОДИН раз в начале обхода
(`process_run` → `role_rewrite_`/`role_seo_`/`role_combined_`) и переиспользуется
для всех новостей — модель не получает одинаковые инструкции повторно
(промпт-роль: экономия токенов/квоты, меньше перегрузка при rate-limit). Если
хост поддерживает `llm_complete_ex`, роль шлётся как system-сообщение; иначе
(старый хост) роль+контент объединяются в один промпт. Подстановки в шаблоне:
`{title}`, `{body}`, `{language}`, `{tone}`, `{max_words}`. Ответ разбивается:
первая непустая строка — заголовок (опционально), остальное — тело.
