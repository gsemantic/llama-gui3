...
| 6 | Расширения (отправка на сервер и пр.) | 6.1 HttpSink, 6.2 прокси/авторизация, 6.3 extractor по плотности — **реализованы**; 6.4 режим `article` + SEO-ссылки (`preserve_external`, `internal_related`) — **реализовано** |
...
- Кнопка **«Обойти сейчас»** — запуск полного цикла fetch→extract→rewrite:
  ...
  - `page` — по маркерам или эвристике по плотности текста (заголовок
    `<h1>`/`<title>`, тело — самый длинный связный набор абзацев-«прозы», без
    nav/header/footer/aside/form и заголовков) извлекается текст страницы.
  - `article` — пользователь указывает URL **конкретной статьи-оригинала**;
    плагин обрабатывает её как одиночную страницу (извлечение текста + всех
    внешних ссылок), рерайтит и публикует одну статью. Режим удобен, когда не
    нужен обход ленты/списка целиком.
...
- **SEO-ссылки в выходном материале** (настройки в `config.links`):
  - `preserve_external` — сохранять **все внешние ссылки оригинала** в блоке
    «Источники» (в конце статьи). Полезно для перелинковки и прозрачности.
    Для источников типа `article` включается **принудительно** (независимо
    от флага), чтобы выходная статья всегда хранила источники.
  - `internal_related` — добавлять **внутренние ссылки на похожие материалы**
    выходного сайта в блоке «Читайте также». Работает только для приёмника
    WordPress (через REST API `/wp-json/wp/v2/...`): по сгенерированным тегам
    статьи (либо `seo_focus_keyword` как запасной вариант) подбирается до
    1–2 похожих записей; если подходящих нет — блок не добавляется.
  - `internal_related_max` — сколько внутренних ссылок добавлять (1 или 2,
    по умолчанию 2).
  - Все ссылки (внешние и внутренние) выводятся с `target="_blank"` и
    `rel="noopener"`, то есть открываются в **новой вкладке**.
...
src/extractor.*       — HTML→текст, title/body по маркерам/эвристике плотности (этапы 2, 6.3)
