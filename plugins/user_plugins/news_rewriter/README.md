...
| 2 | Extractor (HTML → текст) | запланирован |
...
- Кнопка **«Обойти сейчас»** — запуск обхода. На этапе 1 fetch работает:
  источники типа `rss`/`atom` загружаются и разбираются, статьи появляются
  в списке с полным pipeline-контентом (до рерайта). Для `page` возвращается
  сырой HTML (извлечение текста — этап 2).
...
tests/                — юнит-тесты (json, http, xml, fetcher, worker)
```
