...
- [x] 6.1. **Слить теневые .md навыки с inline-версиями, затем удалить**
  - Проведён аудит: inline-навыки в `wp_tools.cpp` УЖЕ содержат весь уникальный
    контент .md (media: image-size/meta/regenerate; boilerplate: ABSPATH/wp_-префикс;
    git: status/diff/log + осмысленные коммиты; hook: приоритеты/shortcode и т.д.)
  - 6 теневых .md удалены (`git rm`) — остался только `wp_setup.md` (реальный внешний)
  - README обновлён: 14 навыков (13 inline + 1 внешний)

- [x] 6.2. **Обновить README.md**
  - Таблица инструментов актуализирована (49: лист_dir, web_fetch, edit_file и др.)
  - Секция «Безопасность» переписана: вместо «известных проблем» — факт реализации Фазы 1
  - Добавлено окно «Сессия» (Ctrl+Shift+S) в таблицу окон; счётчики тестов/навыков исправлены
  - README = краткий обзор; USAGE = подробное руководство (разделение сохранено)

- [x] 6.3. **Обновить CHANGELOG.md**
  - Добавлена секция [0.4.0] - 2026-09-15: Added/Fixed/Changed/Stats по Фазам 1–5

- [x] 6.4. **USAGE.md — подробное руководство (не дубликат README)**
  - Решено: USAGE остаётся, добавлены опции агента (лимит шагов/бюджет),
    секция Resume сессии, окно «Сессия» в таблицу
