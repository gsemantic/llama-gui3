...
AgentResult CodeAgent::handle_explain(const AgentRequest& request) {
    ...
    if (!context_ || !context_->has_llm()) {
        ...
        return AgentResult(AgentStatus::OK, "Basic analysis (LLM not available)")
                .with("explanation", explanation.str())
                .with("language", language);
    }
    
    std::string system_prompt =
        "You are a code analyst. Explain the following code clearly and concisely. "
        "Cover: what it does, key functions, data flow, and any notable patterns.";
    
    std::string user_prompt = "Language: " + language + "\n\n```" + code + "```";
    std::string response = context_->llm_complete(system_prompt, user_prompt);
    
    if (response.empty()) {
        return AgentResult::error("LLM returned empty response");
    }
    
    return AgentResult(AgentStatus::OK, "Code explained via LLM")
...
AgentResult CodeAgent::handle_refactor(const AgentRequest& request) {
    ...
    std::string instructions = request.get_param<std::string>("instructions", "");
    ...
    if (!context_ || !context_->has_llm()) {
        return AgentResult::error("LLM not available for refactoring");
    }
    
    std::string system_prompt =
        "You are an expert code refactorer. Refactor the given code to improve "
        "readability, maintainability, and performance. Follow the style: " + style + ". "
        "Output ONLY the refactored code, no explanations.";
    
    std::string user_prompt = code;
    ...
    return AgentResult(AgentStatus::OK, "Code refactored via LLM")
            .with("code", response)
            .with("style", style);
}
...
AgentResult CodeAgent::handle_translate(const AgentRequest& request) {
    ...
    if (!context_ || !context_->has_llm()) {
        return AgentResult::error("LLM not available for translation");
    }
    
    std::string system_prompt =
        "You are an expert programmer. Translate the following code from " +
        (from_lang.empty() ? "its original language" : from_lang) +
        " to " + to_lang + ". Output ONLY the translated code, no explanations.";
    
    std::string response = context_->llm_complete(system_prompt, code);
    
    if (response.empty()) {
        return AgentResult::error("LLM returned empty response");
    }
    
    return AgentResult(AgentStatus::OK, "Code translated via LLM")
            .with("code", response)
            .with("from", from_lang)
            .with("to", to_lang);
}
...
AgentResult CodeAgent::generate_via_llm(const std::string& prompt,
    ...
    if (!context_ || !context_->has_llm()) {
        return AgentResult::error("LLM not available");
    }

    std::string lang_name;
    switch (language) {
        case CodeLanguage::CPP: lang_name = "C++"; break;
        case CodeLanguage::PYTHON: lang_name = "Python"; break;
        case CodeLanguage::JAVASCRIPT: lang_name = "JavaScript"; break;
        case CodeLanguage::TYPESCRIPT: lang_name = "TypeScript"; break;
        case CodeLanguage::JAVA: lang_name = "Java"; break;
        case CodeLanguage::GO: lang_name = "Go"; break;
        case CodeLanguage::RUST: lang_name = "Rust"; break;
        default: lang_name = "the requested language"; break;
    }

    std::string system_prompt =
        "You are an expert programmer. Generate clean, well-documented code. "
        "Output ONLY the code, no explanations unless explicitly asked. "
        "Use proper indentation and follow best practices for " + lang_name + ".";

    std::string response = context_->llm_complete(system_prompt, prompt);

    if (response.empty()) {
        return AgentResult::error("LLM returned empty response");
    }

    return AgentResult(AgentStatus::OK, "Code generated via LLM")
            .with("code", response)
