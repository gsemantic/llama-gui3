...
// Check if path is within project root (via AgentContext)
AgentResult check_project_root(AgentContext* ctx, const std::string& path) {
    if (ctx && !ctx->get_project_root().empty() && !ctx->is_within_project(path)) {
        return AgentResult::error(
            "Access denied: path is outside the project directory.\n"
            "Path: " + path + "\n"
            "Project root: " + ctx->get_project_root() + "\n"
            "If you need to edit this file, ask the user to expand the project root.");
    }
...
AgentResult FileAgent::handle_read(const AgentRequest& request) {
    ...
    auto root_check = check_project_root(context_, file_path);
    if (!root_check.is_ok()) return root_check;
