...
AgentResult SummarizationAgent::llm_summarize(const std::string& text,
    ...
    if (!context_ || !context_->has_llm()) {
        return AgentResult::error("LLM not available. Use extractive summarization instead.");
    }

    std::string system_prompt =
        "You are a text summarizer. Summarize the following text" +
        (style.empty() ? "" : " in " + style + " style") +
        ". Be concise but comprehensive. Keep key information.";

    // Truncate if too long for LLM context
    std::string input = text;
    if (input.size() > 8000) {
        input = input.substr(0, 8000) + "\n\n[...truncated...]";
    }

    std::string response = context_->llm_complete(system_prompt, input);

    if (response.empty()) {
        return AgentResult::error("LLM returned empty response");
    }

    return AgentResult(AgentStatus::OK, "Summary generated via LLM")
            .with("summary", response)
            .with("method", "llm")
            .with("style", style);
}
